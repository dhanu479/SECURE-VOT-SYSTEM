...
def face_verify():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Ensure exactly one face is detected (use same hog model as registration)
            face_locations = face_recognition.face_locations(face_img, model="hog")
            if len(face_locations) != 1:
                msg = f"Please ensure exactly one face is clearly visible (detected {len(face_locations)} face(s))."
                flash(msg, "danger")
                return redirect(url_for('face_verify'))
            ...
            # Compare with stored face - use 0.6 tolerance (default, more forgiving)
            distance = face_recognition.face_distance([stored], face_encoding)[0]
            print(f"Face distance: {distance:.4f} (threshold: 0.6)")
            match = bool(distance <= 0.6)
            if not match:
                flash(f"Face does not match. Distance: {distance:.2f} (max allowed: 0.60). Ensure good lighting and remove glasses.", "danger")
                return redirect(url_for('face_verify'))
            ...
            traceback.print_exc()
            flash(f"Verification error: {str(e)}. Please try again.", "danger")
            return redirect(url_for('face_verify'))
